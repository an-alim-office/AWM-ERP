...
// Ensure lastOrderDate exists on all (avoid undefined)
MOCK_SUPPLIERS.forEach(s => { if (!s.lastOrderDate) s.lastOrderDate = "2024-12-01"; });
...
/* ============================================================
   MAIN COMPONENT
   ============================================================ */
const SmartPharmacyHub: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const results: SearchResult[] = [
      ...products.filter(p => p.name.toLowerCase().includes(q) || p.sku.toLowerCase().includes(q) || p.batchNumber.toLowerCase().includes(q))
        .map(p => ({ type: "product" as const, id: p.id, title: p.name, subtitle: `SKU: ${p.sku} • Batch: ${p.batchNumber}`, status: p.status, icon: "💊" })),
      ...patients.filter(pt => pt.name.toLowerCase().includes(q) || pt.nationalId.includes(q))
        .map(pt => ({ type: "patient" as const, id: pt.id, title: pt.name, subtitle: `ID: ${pt.nationalId} • Refill: ${pt.nextRefillDate}`, status: pt.alertStatus, icon: "👤" })),
      ...suppliers.filter(s => s.name.toLowerCase().includes(q))
        .map(s => ({ type: "supplier" as const, id: s.id, title: s.name, subtitle: `${s.totalOrders} orders • Score ${s.overallScore}%`, status: s.status, icon: "🚚" })),
    ];
  ...
  const runAICalculation = useCallback(async () => {
    ...
    setAiCalcResult({
      ...
      recommendation: margin < 25 ? "⚠️ Margin below target. AI recommends 8% price increase to meet 30% threshold."
        : margin > 50 ? "✅ Strong margin. Consider volume discount to boost turnover rate."
          : "✅ Optimal pricing. Maintain current strategy with quarterly review.",
    });
    setAiCalcLoading(false);
  }, [aiCalcInput]);

  const triggerReplenishment = useCallback((productId: string) => {
    setReplenishTriggered(prev => prev.includes(productId) ? prev : [...prev, productId]);
    const prod = products.find(p => p.id === productId);
    setNotifications(prev => [{ id: `rep-${productId}-${Date.now()}`, msg: `Auto-replenishment PO generated for ${prod?.name ?? productId}`, level: "success" }, ...prev]);
  }, [products]);

  const generateZATCAInvoice = useCallback(() => {
    setInvoiceModal({
      invoiceNumber: `INV-2025-${String(Math.floor(Math.random() * 99999)).padStart(5, "0")}`,
      qrCode: "AQdTYW1sZSBCdXNpbmVzcwIPMTUwMDAwMDAwMDAwMDAz",
      vatNumber: "310089474500003",
      timestamp: new Date().toISOString(),
      totalAmount: 2345.75, vatAmount: 351.86, status: "generated",
    });
  }, []);
  ...
  const wastageSummary = useMemo(() => {
    ...
    wastageRecords.forEach(w => { byReason[w.reason] = (byReason[w.reason] ?? 0) + w.wastageValue; });
    return { totalValue: total, totalUnits, byReason, records: wastageRecords.length };
  }, [wastageRecords]);

  const expiryBuckets = useMemo(() => {
    const buckets = { critical: [] as PharmacyProduct[], warning: [] as PharmacyProduct[], watch: [] as PharmacyProduct[] };
    products.forEach(p => {
  ...
  const inputBg = isDark ? "bg-white/5 border-white/10 text-white placeholder-white/30" : "bg-slate-100 border-slate-200 text-slate-900 placeholder-slate-400";
  ...
  /* ============================================================
     ...
     ============================================================ */
  return (
    <div className={cn("min-h-screen font-sans antialiased transition-colors duration-300", bg, text)}
      style={{ fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>

      {isDark && (
        <div className="fixed inset-0 pointer-events-none" style={{
          backgroundImage: "radial-gradient(ellipse 80% 50% at 50% -10%, rgba(99,102,241,0.12) 0%, transparent 60%), linear-gradient(rgba(255,255,255,0.015) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.015) 1px, transparent 1px)",
      ...
      {invoiceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setInvoiceModal(null)}>
          <div className={cn("rounded-3xl p-8 max-w-md w-full border shadow-2xl", isDark ? "bg-[#111827]" : "bg-white", border)} onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="ml-auto"><StatusBadge status="compliant" /></span>
            </div>
            <div className={cn("space-y-3 text-sm", subtext)}>
              {[["VAT Number", invoiceModal.vatNumber], ["Invoice No.", invoiceModal.invoiceNumber], ["Timestamp", new Date(invoiceModal.timestamp).toLocaleString("en-SA")], ["Base", formatSAR(invoiceModal.totalAmount)], ["VAT (15%)", formatSAR(invoiceModal.vatAmount)], ["Total", formatSAR(invoiceModal.totalAmount + invoiceModal.vatAmount)]].map(([k, v]) => (
                <div key={k} className={cn("flex justify-between py-2 border-b", border)}>
                  <span>{k}</span><span className={cn("font-semibold", text)}>{v}</span>
                </div>
              ))}
            </div>
            <div className={cn("mt-4 p-3 rounded-xl text-center", isDark ? "bg-white/5" : "bg-slate-50")}>
              ...
              <div className="inline-flex items-center justify-center w-24 h-24 bg-white rounded-lg">
                <div className="w-20 h-20 bg-slate-900 rounded" style={{
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setInvoiceModal(null); setNotifications(prev => [{ id: `zx-${Date.now()}`, msg: `Invoice submitted to ZATCA portal`, level: "success" }, ...prev]); }}
                className="flex-1 py-2.5 rounded-xl bg-emerald-500 text-white text-sm font-semibold hover:bg-emerald-600 transition-colors">Submit to ZATCA</button>
              <button onClick={() => setInvoiceModal(null)}
                className={cn("flex-1 py-2.5 rounded-xl text-sm font-semibold transition-colors", softBg, hoverBg2)}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {contactModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setContactModal(null)}>
          <div className={cn("rounded-3xl p-6 max-w-md w-full border shadow-2xl", isDark ? "bg-[#111827]" : "bg-white", border)} onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white text-lg font-bold">🚚</div>
            ...
            <div className="space-y-2 text-sm">
              {[["Contact Person", contactModal.contactPerson], ["Phone", contactModal.phone], ["Email", contactModal.email], ["VAT No.", contactModal.vatNumber], ["Payment Terms", contactModal.paymentTerms], ["Total Value", formatSAR(contactModal.totalValue)], ["Overall Score", `${contactModal.overallScore}%`]].map(([k, v]) => (
                <div key={k} className={cn("flex justify-between py-2 border-b", border)}>
                  <span className={subtext}>{k}</span><span className="font-semibold">{v}</span>
                </div>
              ))}
            </div>
            <div className="flex gap-2 mt-5">
              <button onClick={() => { setNotifications(prev => [{ id: `email-${Date.now()}`, msg: `Email sent to ${contactModal.contactPerson}`, level: "success" }, ...prev]); setContactModal(null); }}
                className="flex-1 py-2 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-semibold">📧 Send Email</button>
              <button onClick={() => { setNotifications(prev => [{ id: `po-${Date.now()}`, msg: `PO drafted for ${contactModal.name}`, level: "info" }, ...prev]); setContactModal(null); }}
                className="flex-1 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold">📝 Create PO</button>
              <button onClick={() => setContactModal(null)} className={cn("py-2 px-3 rounded-xl text-xs font-semibold", softBg, hoverBg2)}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {wastageModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setWastageModal(null)}>
          <div className={cn("rounded-3xl p-6 max-w-md w-full border shadow-2xl", isDark ? "bg-[#111827]" : "bg-white", border)} onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="space-y-2 text-sm">
              {[["Product", wastageModal.productName], ["Batch", wastageModal.batchNumber], ["Quantity", `${wastageModal.quantity} units`], ["Value Loss", formatSAR(wastageModal.wastageValue)], ["Category", wastageModal.category], ["Recorded By", wastageModal.recordedBy], ["Date", wastageModal.wastageDate], ["Disposal", wastageModal.disposalMethod]].map(([k, v]) => (
                <div key={k} className={cn("flex justify-between py-2 border-b", border)}>
                  <span className={subtext}>{k}</span><span className="font-semibold text-right">{v}</span>
                </div>
              ))}
            </div>
            <button onClick={() => setWastageModal(null)} className={cn("w-full mt-5 py-2 rounded-xl text-xs font-semibold", softBg, hoverBg2)}>Close</button>
          </div>
        </div>
      )}
        ...
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4 h-16">
            ...
            <div className="flex-1 max-w-2xl mx-auto relative">
              <div className={cn("flex items-center gap-2 rounded-xl border px-3 py-2 transition-all duration-200", searchFocused ? "border-indigo-500/60 ring-2 ring-indigo-500/15" : border, isDark ? "bg-white/5" : "bg-slate-50")}>
                <span className="text-indigo-400 text-sm">🔍</span>
                <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-md shrink-0", isDark ? "bg-indigo-500/15 text-indigo-400" : "bg-indigo-50 text-indigo-600")}>AI</span>
                <input ref={searchRef} type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  ...
                  className={cn("flex-1 bg-transparent text-sm outline-none", isDark ? "text-white placeholder-white/30" : "text-slate-900 placeholder-slate-400")}
                  aria-label="AI search" />
                {searchQuery && <button onClick={() => setSearchQuery("")} className={cn(subtext, "hover:text-red-400 transition-colors")}>✕</button>}
                <kbd className={cn("hidden sm:inline text-[10px] px-1.5 py-0.5 rounded border", border, subtext)}>⌘K</kbd>
              </div>

              {searchFocused && searchResults.length > 0 && (
                <div className={cn("absolute top-full mt-2 left-0 right-0 rounded-2xl border shadow-2xl shadow-black/30 overflow-hidden z-50", border, isDark ? "bg-[#111827]" : "bg-white")}>
                  ...
                  {searchResults.map(r => (
                    <button key={r.id} onMouseDown={e => e.preventDefault()}
                      onClick={() => {
                        if (r.type === "product") { const p = products.find(x => x.id === r.id); if (p) { setActiveTab("inventory"); setSelectedProduct(p); } }
                        if (r.type === "supplier") { const s = suppliers.find(x => x.id === r.id); if (s) { setActiveTab("suppliers"); setSelectedSupplier(s); } }
                        if (r.type === "patient") setActiveTab("patients");
                        setSearchQuery("");
                      }}
                      className={cn("w-full flex items-center gap-3 px-4 py-3 text-left transition-colors border-b last:border-0", border, hoverBg)}>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button className={cn("relative p-2 rounded-xl transition-colors", hoverBg2)} aria-label="Notifications">
                ...
                {notifications.length > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-red-500 text-white text-[9px] font-bold flex items-center justify-center">{notifications.length}</span>
                )}
              </button>
              <button onClick={() => setTheme(t => t === "dark" ? "light" : "dark")}
                className={cn("p-2 rounded-xl transition-colors", hoverBg2)} aria-label="Toggle theme">
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div className="space-y-2">
            {notifications.slice(0, 2).map(n => (
              <div key={n.id} className={cn("flex items-center gap-3 px-4 py-3 rounded-xl border text-sm",
                n.level === "critical" ? "bg-red-500/8 border-red-500/25 text-red-300" :
                n.level === "warning" ? "bg-amber-500/8 border-amber-500/25 text-amber-300" :
                ...
                <button onClick={() => setNotifications(prev => prev.filter(x => x.id !== n.id))} className="opacity-60 hover:opacity-100 text-xs">✕</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className={cn("text-sm mt-0.5", subtext)}>
              AI-Powered • Real-Time • ZATCA Compliant • {new Date().toLocaleDateString("en-SA", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={generateZATCAInvoice} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold transition-colors shadow-lg shadow-emerald-500/25">
              <span>🧾</span> Generate E-Invoice
            </button>
            <button onClick={() => { setNotifications(prev => [{ id: `add-${Date.now()}`, msg: "Add Product dialog would open", level: "info" }, ...prev]); }}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 gap-3">
          {loading ? Array.from({ length: 8 }).map((_, i) => <SkeletonCard key={i} />) : statCards.map(card => (
            <div key={card.label} className={cn("group relative rounded-2xl p-4 border backdrop-blur-sm transition-all duration-200 overflow-hidden", border, cardBg)}>
              ...
              <p className="text-lg font-bold leading-none mb-1" style={{ color: card.color }}>{card.value}</p>
        ...
        <div className={cn("flex items-center gap-1 p-1 rounded-2xl border overflow-x-auto", border, isDark ? "bg-white/3" : "bg-slate-100")}>
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={cn("flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all duration-200",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className={cn("lg:col-span-1 rounded-2xl border p-5 backdrop-blur-sm", border, cardBg)}>
              ...
              <div className="space-y-3">
                {notifications.map(n => (
                  <div key={n.id} className={cn("p-3 rounded-xl border text-xs",
                    n.level === "critical" ? "bg-red-500/8 border-red-500/20" :
                    n.level === "warning" ? "bg-amber-500/8 border-amber-500/20" :
                    "bg-emerald-500/8 border-emerald-500/20")}>
                    <p className={n.level === "critical" ? "text-red-300" : n.level === "warning" ? "text-amber-300" : "text-emerald-300"}>{n.msg}</p>
                  </div>
                ))}
                {notifications.length === 0 && (
                  <div className="text-center py-8"><span className="text-3xl">✅</span>
                    <p className={cn("text-sm mt-2", subtext)}>All systems normal</p></div>
                )}
              </div>
            </div>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className={cn("rounded-2xl border p-5 backdrop-blur-sm", border, cardBg)}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-2">
                    {[["E-Invoices", "Cleared", "text-emerald-400"], ["VAT Reports", "Filed", "text-emerald-400"], ["Batch Records", "1 Pending", "text-amber-400"]].map(([l, v, c]) => (
                      <div key={l} className="flex items-center gap-2 text-xs">
                        <span className={subtext}>{l}:</span><span className={cn("font-semibold", c)}>{v}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className={cn("rounded-2xl border p-5 backdrop-blur-sm", border, cardBg)}>
                ...
                <div className="space-y-2">
                  {products.filter(p => p.stockLevel <= p.minThreshold).slice(0, 3).map(p => (
                    <div key={p.id} className={cn("flex items-center gap-2 p-2 rounded-lg", isDark ? "bg-white/4" : "bg-slate-50")}>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium truncate">{p.name}</p>
                        <p className={cn("text-[10px]", subtext)}>{p.stockLevel} / {p.minThreshold} units</p>
                      </div>
                      <button onClick={() => triggerReplenishment(p.id)} disabled={replenishTriggered.includes(p.id)}
                        className={cn("shrink-0 px-2 py-1 rounded-lg text-[10px] font-semibold transition-colors",
                          replenishTriggered.includes(p.id) ? "bg-emerald-500/20 text-emerald-400 cursor-not-allowed" : "bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30")}>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className={cn("border-b", border, isDark ? "bg-white/3" : "bg-slate-50")}>
                      {[
                        { key: "name", label: "Product" }, { key: "sku", label: "SKU / Batch" },
                        { key: "stockLevel", label: "Stock" }, { key: "expiryDate", label: "Expiry" },
                        { key: "sellingPrice", label: "Price" }, { key: "profitMargin", label: "Margin" },
                        { key: "status", label: "Status" }, { key: "zatcaCompliance", label: "ZATCA" },
                      ].map(col => (
                        <th key={col.key} onClick={() => toggleSort(col.key as keyof PharmacyProduct)}
                          className={cn("px-4 py-3 text-left text-[10px] font-bold uppercase tracking-widest cursor-pointer hover:text-indigo-400 transition-colors select-none whitespace-nowrap", subtext)}>
                    ...
                    {loading ? Array.from({ length: 4 }).map((_, i) => (
                      <tr key={i} className="animate-pulse">
                        {Array.from({ length: 9 }).map((__, j) => (
                          <td key={j} className="px-4 py-4"><div className={cn("h-3 rounded", isDark ? "bg-white/8" : "bg-slate-200")} /></td>
                        ))}
                      </tr>
                      ...
                      return (
                        <tr key={product.id} onClick={() => setSelectedProduct(selectedProduct?.id === product.id ? null : product)}
                            ...
                            <div className="flex items-center gap-2">
                              <div className="flex-1 h-1.5 rounded-full bg-white/10 w-16">
                                <div className="h-full rounded-full transition-all"
                                  style={{ width: `${Math.min((product.stockLevel / product.maxThreshold) * 100, 100)}%`,
                                    background: product.stockLevel === 0 ? "#ef4444" : product.stockLevel < product.minThreshold ? "#f59e0b" : "#10b981" }} />
                              </div>
                              <span className="text-xs font-bold">{formatNumber(product.stockLevel)}</span>
                            </div>
                            {forecast && <p className={cn("text-[10px] mt-0.5", subtext)}>AI: {forecast.daysUntilStockout}d left</p>}
                          </td>
                          <td className="px-4 py-3">
                            <p className="text-xs">{product.expiryDate}</p>
                            <p className={cn("text-[10px] font-semibold", daysLeft < 30 ? "text-red-400" : daysLeft < 90 ? "text-amber-400" : subtext)}>
                          ...
                          <td className="px-4 py-3">
                            <p className={cn("text-xs font-bold", product.profitMargin > 40 ? "text-emerald-400" : product.profitMargin > 25 ? "text-amber-400" : "text-red-400")}>{product.profitMargin.toFixed(1)}%</p>
                          </td>
                          <td className="px-4 py-3"><StatusBadge status={product.status} /></td>
                          <td className="px-4 py-3"><StatusBadge status={product.zatcaCompliance} /></td>
                          <td className="px-4 py-3">
                            <button onClick={e => { e.stopPropagation(); triggerReplenishment(product.id); }}
                              ...
                              className={cn("px-2 py-1 rounded-lg text-[10px] font-semibold transition-colors",
                                replenishTriggered.includes(product.id) ? "bg-emerald-500/15 text-emerald-400 cursor-not-allowed" :
                                product.stockLevel <= product.minThreshold ? "bg-amber-500/15 text-amber-400 hover:bg-amber-500/25" :
            ...
            {selectedProduct && (
              <div className={cn("rounded-2xl border border-indigo-500/30 p-5", isDark ? "bg-indigo-500/5" : "bg-indigo-50")}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setSelectedProduct(null)} className={cn("text-sm hover:text-red-400", subtext)}>✕ Close</button>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                  {[["Manufacturer", selectedProduct.manufacturer], ["Category", selectedProduct.category], ["Unit Cost", `SAR ${selectedProduct.unitCost.toFixed(2)}`], ["Selling Price", `SAR ${selectedProduct.sellingPrice.toFixed(2)}`], ["VAT Rate", `${selectedProduct.vatRate}%`], ["Turnover Rate", `${selectedProduct.turnoverRate}x`], ["Min Threshold", formatNumber(selectedProduct.minThreshold)], ["Max Threshold", formatNumber(selectedProduct.maxThreshold)]].map(([k, v]) => (
                    <div key={k}>
                ...
                {forecasts.find(f => f.productId === selectedProduct.id) && (() => {
                  ...
                  return (
                    <div className={cn("mt-4 p-3 rounded-xl border", isDark ? "bg-white/5" : "bg-white", border)}>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-xs">
                        <div><p className={subtext}>Predicted Demand</p><p className="font-bold">{fc.predictedDemand} units</p></div>
                        <div><p className={subtext}>Confidence</p><p className="font-bold text-emerald-400">{fc.confidence}%</p></div>
                        <div><p className={subtext}>Suggested Reorder</p><p className="font-bold">{fc.suggestedReorder} units</p></div>
                        <div><p className={subtext}>Optimal Price</p><p className="font-bold">SAR {fc.priceOptimization}</p></div>
                        <div><p className={subtext}>Trend</p><TrendIndicator trend={fc.trend} /></div>
                      </div>
                    </div>
                  );
                })()}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setNotifications(prev => [{ id: `pt-${Date.now()}`, msg: "Add Patient dialog would open", level: "info" }, ...prev])}
                  className="px-3 py-1.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-semibold">+ Add Patient</button>
              </div>
            </div>
                  ...
                  <div key={patient.id} className={cn("rounded-2xl border p-5 backdrop-blur-sm", border, cardBg)}>
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={cn("text-[10px]", subtext)}>ID: {patient.nationalId} • {patient.insuranceProvider}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className={cn("text-[10px]", subtext)}>Adherence</p>
                        <p className={cn("text-sm font-bold", patient.adherenceScore >= 90 ? "text-emerald-400" : patient.adherenceScore >= 70 ? "text-amber-400" : "text-red-400")}>{patient.adherenceScore}%</p>
                      </div>
                    </div>
                    <div className="space-y-2 mb-4">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {patient.medications.map(med => (
                          <span key={med} className={cn("px-2 py-0.5 rounded-lg text-[10px] font-medium border border-indigo-500/20", isDark ? "bg-indigo-500/15 text-indigo-300" : "bg-indigo-50 text-indigo-700")}>{med}</span>
                        ))}
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      <div>
                        ...
                        <p className={cn("text-xs font-bold", daysUntilRefill < 0 ? "text-red-400" : daysUntilRefill < 3 ? "text-amber-400" : "text-emerald-400")}>
                      ...
                      <div><p className={cn("text-[10px]", subtext)}>Last Visit</p><p className="text-xs font-bold">{patient.lastVisit}</p></div>
                      <div><p className={cn("text-[10px]", subtext)}>Phone</p><p className="text-xs font-bold">{patient.phone}</p></div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => setNotifications(prev => [{ id: `sms-${patient.id}-${Date.now()}`, msg: `SMS refill alert sent to ${patient.name}`, level: "success" }, ...prev])}
                        className={cn("flex-1 py-2 rounded-xl text-[11px] font-semibold transition-colors",
                          daysUntilRefill <= 3 ? "bg-amber-500/20 text-amber-400 hover:bg-amber-500/30" : cn(softBg, hoverBg2, subtext))}>📲 Send Refill Alert</button>
                      <button className={cn("flex-1 py-2 rounded-xl text-[11px] font-semibold transition-colors", softBg, hoverBg2, subtext)}>📋 View History</button>
                      <button onClick={() => setNotifications(prev => [{ id: `dsp-${patient.id}-${Date.now()}`, msg: `Dispensed medication to ${patient.name}`, level: "success" }, ...prev])}
                        className="flex-1 py-2 rounded-xl text-[11px] font-semibold bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30 transition-colors">🧾 Dispense</button>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            <div className={cn("lg:col-span-2 rounded-2xl border p-6 backdrop-blur-sm", border, cardBg)}>
              <div className="flex items-center gap-2 mb-5">
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-sm">🧮</div>
                <div>
              ...
              <div className="space-y-3 mb-4">
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input type="number" value={aiCalcInput[f.key as keyof typeof aiCalcInput]}
                      onChange={e => setAiCalcInput(prev => ({ ...prev, [f.key]: e.target.value }))}
                      placeholder={f.ph}
                      className={cn("w-full px-3 py-2.5 rounded-xl border text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15 transition-all", inputBg)} />
                  </div>
                ))}
              </div>
              <button onClick={runAICalculation} disabled={aiCalcLoading}
                ...
                <div className={cn("mt-4 rounded-xl border p-4 space-y-3", border, isDark ? "bg-white/4" : "bg-slate-50")}>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      { l: "Profit Margin", v: `${aiCalcResult.profitMargin.toFixed(1)}%`, c: aiCalcResult.profitMargin > 30 ? "text-emerald-400" : "text-amber-400" },
                  ...
                  <div className={cn("rounded-lg p-2.5 border", isDark ? "border-white/8 bg-white/3" : "border-slate-200 bg-white")}>
                    ...
                    <div className="flex justify-between text-xs">
                      <span className={subtext}>Base: <span className={text}>SAR {aiCalcResult.vatBreakdown.base.toFixed(2)}</span></span>
                      <span className={subtext}>VAT: <span className="text-amber-400">SAR {aiCalcResult.vatBreakdown.vat.toFixed(2)}</span></span>
                      <span className={subtext}>Total: <span className="text-emerald-400 font-bold">SAR {aiCalcResult.vatBreakdown.total.toFixed(2)}</span></span>
                    </div>
                  </div>
            ...
            <div className={cn("lg:col-span-3 rounded-2xl border p-6 backdrop-blur-sm", border, cardBg)}>
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="text-[10px] px-2 py-1 rounded-lg bg-indigo-500/15 text-indigo-400 font-semibold border border-indigo-500/20">Model: Prophet + LSTM</span>
              </div>
              <div className="space-y-3">
                {forecasts.map(fc => {
                    ...
                    <div key={fc.productId} className={cn("rounded-xl border p-4 transition-colors", border, hoverBg)}>
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <span className={cn("text-[10px] font-semibold", fc.daysUntilStockout <= 3 ? "text-red-400" : fc.daysUntilStockout <= 14 ? "text-amber-400" : "text-emerald-400")}>
                            {fc.daysUntilStockout === 0 ? "STOCKOUT" : `${fc.daysUntilStockout}d left`}
                          </span>
                        </div>
                      </div>
                      <div className="grid grid-cols-4 gap-3 mb-3">
                        {[["Predicted Demand", `${fc.predictedDemand} units`], ["AI Confidence", `${fc.confidence}%`], ["Suggested Reorder", `${fc.suggestedReorder} units`], ["Optimal Price", `SAR ${fc.priceOptimization}`]].map(([l, v]) => (
                      ...
                      <div className="flex items-center gap-2">
                        <div className={cn("flex-1 h-1.5 rounded-full", isDark ? "bg-white/8" : "bg-slate-200")}>
                          <div className="h-full rounded-full transition-all" style={{ width: `${fc.confidence}%`, background: fc.confidence > 90 ? "#10b981" : fc.confidence > 75 ? "#f59e0b" : "#ef4444" }} />
                        </div>
                        <span className={cn("text-[10px]", subtext)}>{fc.confidence.toFixed(1)}% confident</span>
                        {fc.suggestedReorder > 0 && (
                          <button onClick={() => triggerReplenishment(fc.productId)} disabled={replenishTriggered.includes(fc.productId)}
                            className={cn("ml-2 px-2.5 py-1 rounded-lg text-[10px] font-semibold transition-colors",
                              replenishTriggered.includes(fc.productId) ? "bg-emerald-500/15 text-emerald-400 cursor-not-allowed" : "bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30")}>
        ...
        {activeTab === "zatca" && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={card.l} className={cn("rounded-2xl border p-5 flex items-center gap-4", border, cardBg)}>
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0" style={{ background: `${card.c}20` }}>{card.i}</div>
                  <div>
                    <p className={cn("text-[10px] uppercase font-semibold tracking-wider", subtext)}>{card.l}</p>
                    <p className="text-xl font-bold" style={{ color: card.c }}>{card.v}</p>
            ...
            <div className={cn("rounded-2xl border p-6 backdrop-blur-sm", border, cardBg)}>
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-sm font-bold flex items-center gap-2">
                  <span className="w-6 h-6 rounded-lg bg-blue-500/20 flex items-center justify-center">🧾</span>Recent ZATCA Transactions
                </h2>
                <button onClick={generateZATCAInvoice} className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold transition-colors">+ Generate E-Invoice</button>
              </div>
              <div className="overflow-x-auto">
                  ...
                  <thead>
                    <tr className={cn("border-b", border)}>
                      {["Invoice No.", "Date & Time", "Base", "VAT (15%)", "Total", "Status", "QR"].map(h => (
                        <th key={h} className={cn("px-4 py-2.5 text-left text-[10px] font-bold uppercase tracking-widest", subtext)}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className={`divide-y ${border}`}>
                    {[
                      { no: "INV-2025-00891", date: "2025-01-08 14:23", base: 2345.75, status: "cleared" },
                      { no: "INV-2025-00890", date: "2025-01-08 13:45", base: 1820, status: "cleared" },
                      { no: "INV-2025-00889", date: "2025-01-08 12:10", base: 550.5, status: "submitted" },
                      { no: "INV-2025-00888", date: "2025-01-08 11:32", base: 3200, status: "cleared" },
                      { no: "INV-2025-00887", date: "2025-01-08 09:55", base: 789.25, status: "generated" },
                    ].map(inv => {
                        ...
                        <tr key={inv.no} className={cn("transition-colors", hoverBg)}>
                          ...
                          <td className="px-4 py-3">
                            <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-bold border",
                              inv.status === "cleared" ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" :
                              inv.status === "submitted" ? "bg-blue-500/15 text-blue-400 border-blue-500/30" :
                              "bg-amber-500/15 text-amber-400 border-amber-500/30")}>
                              {inv.status.charAt(0).toUpperCase() + inv.status.slice(1)}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <button onClick={generateZATCAInvoice} className="text-indigo-400 hover:text-indigo-300 text-[10px] font-semibold">View QR</button>
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div className={cn("rounded-2xl border p-5", border, cardBg)}>
              <h3 className="text-sm font-bold mb-4 flex items-center gap-2">
                <span className="w-6 h-6 rounded-lg bg-amber-500/20 flex items-center justify-center text-xs">⚙️</span>ZATCA Compliance Rules Active
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  ["Standard VAT Rate", "15%"], ["E-Invoice", "Phase 2 Fatoorah"], ["QR Code", "TLV Encoded"],
                  ["Real-time API", "Connected"], ["Crypto Stamp", "ECDSA P-256"], ["Arabic Tax Desc.", "Required"],
                  ["VAT Number", "310089474500003"], ["Clearance", "≥ SAR 1,000"],
                ].map(([r, v]) => (
                  <div key={r} className={cn("p-3 rounded-xl border", border, isDark ? "bg-white/3" : "bg-slate-50")}>
                    ...
                    <span className="inline-block mt-1 text-[9px] font-bold text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded-full border border-emerald-500/20">✓ Active</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { l: "Avg On-Time %", v: `${(suppliers.reduce((s, x) => s + x.onTimeDelivery, 0) / (suppliers.length || 1)).toFixed(1)}%`, c: "#06b6d4", i: "⏱️" },
              ].map(c => (
                <div key={c.l} className={cn("rounded-2xl border p-4 flex items-center gap-3", border, cardBg)}>
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center text-lg shrink-0" style={{ background: `${c.c}20` }}>{c.i}</div>
                  <div className="min-w-0">
                    <p className={cn("text-[10px] uppercase font-semibold tracking-wider", subtext)}>{c.l}</p>
                    <p className="text-lg font-bold truncate" style={{ color: c.c }}>{c.v}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {filteredSuppliers.map((s, idx) => (
                <div key={s.id} className={cn("rounded-2xl border p-5 backdrop-blur-sm transition-all",
                  selectedSupplier?.id === s.id ? "border-indigo-500/50 ring-2 ring-indigo-500/15" : border, cardBg)}>
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-start gap-3 min-w-0 flex-1">
                      <div className="relative">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white text-lg font-bold shrink-0">🚚</div>
                        <div className={cn("absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black text-white",
                          idx === 0 ? "bg-yellow-500" : idx === 1 ? "bg-slate-400" : idx === 2 ? "bg-amber-600" : "bg-slate-600")}>#{idx + 1}</div>
                      </div>
                    ...
                    <div className="relative shrink-0">
                      <ProgressRing value={s.overallScore} size={54} stroke={5}
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    {[
                      { l: "On-Time Delivery", v: `${s.onTimeDelivery}%`, c: s.onTimeDelivery > 90 ? "#10b981" : s.onTimeDelivery > 80 ? "#f59e0b" : "#ef4444" },
                      ...
                      <div key={m.l} className={cn("rounded-lg p-2 border", border, isDark ? "bg-white/3" : "bg-slate-50")}>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <div className={cn("flex-1 h-1 rounded-full", isDark ? "bg-white/8" : "bg-slate-200")}>
                            <div className="h-full rounded-full transition-all"
                              style={{ width: `${Math.min(parseFloat(String(m.v).replace("%", "")) * (m.l === "Return Rate" ? 10 : 1), 100)}%`, background: m.c }} />
                          </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4 text-xs">
                    <div><p className={cn("text-[10px]", subtext)}>Orders</p><p className="font-bold">{s.totalOrders}</p></div>
                    <div><p className={cn("text-[10px]", subtext)}>Value</p><p className="font-bold">{formatSAR(s.totalValue).replace("SAR", "")}</p></div>
                    <div><p className={cn("text-[10px]", subtext)}>Response</p><p className="font-bold">{s.avgResponseHours}h</p></div>
                  </div>

                  <div className="flex gap-2">
                    <button onClick={() => setContactModal(s)}
                      className={cn("flex-1 py-2 rounded-xl text-[11px] font-semibold transition-colors", softBg, hoverBg2)}>📞 Contact</button>
                    ...
                    <button onClick={() => { setNotifications(prev => [{ id: `po-${s.id}-${Date.now()}`, msg: `New PO drafted for ${s.name}`, level: "success" }, ...prev]); }}
                      className="flex-1 py-2 rounded-xl text-[11px] font-semibold bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 transition-colors">📝 New PO</button>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className={cn("rounded-2xl border border-indigo-500/30 p-5", isDark ? "bg-indigo-500/5" : "bg-indigo-50")}>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className={cn("text-[11px]", subtext)}>{selectedSupplier.email} • Last order: {selectedSupplier.lastOrderDate}</p>
                  </div>
                  <button onClick={() => setSelectedSupplier(null)} className={cn("text-sm hover:text-red-400", subtext)}>✕ Close</button>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className={cn("lg:col-span-2 rounded-2xl border p-5", border, cardBg)}>
                <h3 className="text-sm font-bold flex items-center gap-2 mb-4">
                  <span className="w-6 h-6 rounded-lg bg-red-500/20 flex items-center justify-center text-xs">📅</span>Expiry Pipeline (Next 6 Months)
                </h3>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={bucket.label}>
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-xs font-semibold" style={{ color: bucket.color }}>{bucket.label}</p>
                        <span className={cn("text-[10px]", subtext)}>{bucket.items.length} items</span>
                      </div>
                      {bucket.items.length === 0 ? (
                        <div className={cn("text-center py-3 rounded-lg text-[11px]", subtext, isDark ? "bg-white/3" : "bg-slate-50")}>None in this range ✓</div>
                      ) : (
                          ...
                          {bucket.items.slice(0, 4).map(p => {
                            ...
                            return (
                              <div key={p.id} className={cn("flex items-center gap-2 p-2 rounded-lg", isDark ? "bg-white/3" : "bg-slate-50")}>
                                <div className="flex-1 min-w-0">
                                  <p className="text-xs font-medium truncate">{p.name}</p>
                                  <p className={cn("text-[10px] truncate", subtext)}>{p.batchNumber} • Stock: {p.stockLevel}</p>
                                </div>
                                <div className="text-right shrink-0">
                                  <p className="text-xs font-bold" style={{ color: bucket.color }}>{days}d</p>
                                  <p className={cn("text-[10px]", subtext)}>{p.expiryDate}</p>
                                </div>
                                <button onClick={() => setNotifications(prev => [{ id: `mark-${p.id}-${Date.now()}`, msg: `${p.name} marked for review`, level: "info" }, ...prev])}
              ...
              <div className={cn("rounded-2xl border p-5", border, cardBg)}>
                <h3 className="text-sm font-bold flex items-center gap-2 mb-4">
                  <span className="w-6 h-6 rounded-lg bg-violet-500/20 flex items-center justify-center text-xs">📊</span>Wastage by Reason
                </h3>
                <div className="space-y-3">
                  {Object.entries(wastageSummary.byReason).map(([reason, value]) => {
                    const pct = wastageSummary.totalValue > 0 ? (value / wastageSummary.totalValue) * 100 : 0;
                    const colors: Record<string, string> = { expired: "#ef4444", damaged: "#f59e0b", recall: "#8b5cf6", temperature: "#06b6d4", other: "#64748b" };
                    return (
                      <div key={reason}>
                        ...
                        <div className={cn("h-2 rounded-full overflow-hidden", isDark ? "bg-white/8" : "bg-slate-200")}>
                          <div className="h-full rounded-full transition-all"
                            style={{ width: `${pct}%`, background: colors[reason] || "#64748b" }} />
                        </div>
            ...
            <div className={cn("rounded-2xl border p-5", border, cardBg)}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold flex items-center gap-2">
                  <span className="w-6 h-6 rounded-lg bg-red-500/20 flex items-center justify-center text-xs">📋</span>Wastage Log & Disposal Ledger
                </h3>
                <div className="flex items-center gap-2 overflow-x-auto">
                  {["all", "expired", "damaged", "recall", "temperature"].map(f => (
                    <button key={f} onClick={() => setWastageFilter(f)}
                      className={cn("px-2.5 py-1 rounded-lg text-[10px] font-semibold whitespace-nowrap transition-colors",
                  ...
                  <tbody className={`divide-y ${border}`}>
                    {filteredWastage.map(w => (
                      <tr key={w.id} className={cn("transition-colors", hoverBg)}>
                        ...
                        <td className="px-4 py-3"><StatusBadge status={w.reason} /></td>
                        <td className={cn("px-4 py-3 text-[10px]", subtext)}>{w.disposalMethod}</td>
                        <td className="px-4 py-3">
                          <button onClick={() => setWastageModal(w)}
                            className="text-indigo-400 hover:text-indigo-300 text-[10px] font-semibold">View</button>
                        </td>
                      </tr>
                    ))}
                    {filteredWastage.length === 0 && (
                      <tr><td colSpan={8} className={cn("px-4 py-8 text-center", subtext)}>No wastage records in this filter.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="mt-4 flex items-center justify-between pt-4 border-t border-white/8">
                <p className={cn("text-[11px]", subtext)}>
                  Total loss this cycle: <span className="text-red-400 font-bold">{formatSAR(filteredWastage.reduce((s, w) => s + w.wastageValue, 0))}</span>
                </p>
                <div className="flex gap-2">
                  <button onClick={() => setNotifications(prev => [{ id: `wxp-${Date.now()}`, msg: "Wastage report exported to CSV", level: "success" }, ...prev])}
                    className={cn("px-3 py-1.5 rounded-lg text-[10px] font-semibold transition-colors", softBg, hoverBg2)}>📊 Export CSV</button>
                  <button onClick={() => setNotifications(prev => [{ id: `wrp-${Date.now()}`, msg: "New wastage entry dialog would open", level: "info" }, ...prev])}
                    className="px-3 py-1.5 rounded-lg text-[10px] font-semibold bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors">+ Log Wastage</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <footer className={cn("border-t pt-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-[10px]", border, subtext)}>
          <p>Smart Pharmacy Hub v3.3 • ERP Suite 2026 • ZATCA Phase 2 compliant</p>
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5">
              <span className={cn("w-1.5 h-1.5 rounded-full bg-emerald-400 transition-opacity", pulse ? "opacity-100" : "opacity-40")} />Real-time sync
            </span>
