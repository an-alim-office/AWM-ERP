...
function getSeedRows(): AdvanceSheetRow[] {
  ...
  return seed.map((row) => ({
    ...row,
    netAmount: recalculateNetAmount(row),
  }));
}
...
async function seedIfEmpty() {
  ...
  if (count === 0) {
    await collection.insertMany(getSeedRows());
  }
}

function stripMongoId(doc: AdvanceSheetRow & { _id?: ObjectId }): AdvanceSheetRow {
  const { _id, ...rest } = doc;
  return rest;
}
...
async function getDeleteId(request: NextRequest) {
  ...
  const bodyId = body.id || body.recordId || body.employeeId || body._id;
  return typeof bodyId === "string" ? bodyId : "";
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!payload.id || typeof payload.id !== "string") {
      return jsonResponse({ ok: false, message: "Valid employee ID is required." }, 400);
    }

    if (!payload.name || typeof payload.name !== "string") {
      return jsonResponse({ ok: false, message: "Employee name is required." }, 400);
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!payload.id || typeof payload.id !== "string") {
      return jsonResponse({ ok: false, message: "Valid employee ID is required for update." }, 400);
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const recordId = await getDeleteId(request);

    if (!recordId) {
      return jsonResponse(
        {
          ok: false,
          message: "Record id is required. Send it as ?id=EMP-2026-001 or in the JSON body.",
        },
        400
      );
    }
    ...
    const collection = await getCollection();
    const filters: Filter<AdvanceSheetRow>[] = [{ id: recordId }, { smartId: recordId }];

    if (ObjectId.isValid(recordId)) {
      filters.push({ _id: new ObjectId(recordId) } as Filter<AdvanceSheetRow>);
    }
    ...
    if (!existing) {
      return jsonResponse(
        { ok: false, message: `No advance sheet row found for ID: ${recordId}` },
        404
      );
    }

    const result = await collection.deleteOne({ _id: (existing as { _id: ObjectId })._id });

    if (result.deletedCount !== 1) {
      return jsonResponse(
        { ok: false, message: `Failed to delete advance sheet row for ID: ${recordId}` },
        500
      );
    }
    ...
    return jsonResponse({
      ...
      message: "Advance sheet row deleted successfully.",
      data: stripMongoId(existing),
      allData: allRows,
      summary: buildSummary(allRows),
    });
  } catch (error) {
    return jsonResponse(
      {
        ok: false,
        message: error instanceof Error ? error.message : "Failed to delete staff advance entry.",
      },
      500
    );
  }
