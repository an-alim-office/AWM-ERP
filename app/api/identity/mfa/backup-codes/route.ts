...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const existing = mfaManager.get(tenantId, userId);
    if (!existing) return errorResponse("MFA_NOT_FOUND", "MFA enrollment not found.", 404);

    return NextResponse.json({
      success: true,
      data: { backupCodesHash: existing.backupCodesHash ?? [] },
      meta: meta(),
    } satisfies ApiResponse<{ backupCodesHash: string[] }>, { status: 200 });
  } catch (error: unknown) {
    return errorResponse("MFA_BACKUP_CODES_FAILED", error instanceof Error ? error.message : "Failed to fetch backup codes.", 500);
  }
}