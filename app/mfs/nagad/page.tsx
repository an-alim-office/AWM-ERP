...
export default function NagadPage() {
  const [config, setConfig] = useState<NagadConfig | null>(null);
  ...
  const fetchConfig = async () => {
    try {
      const res = await fetch("/api/mfs/nagad");
  ...
  const fetchTransactions = async () => {
    try {
      const res = await fetch("/api/mfs/nagad?limit=10");
      const data = await res.json();
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch("/api/mfs/nagad/stats");
      const data = await res.json();
      if (res.ok && data.data) {
        setStats(data.data);
      }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Nagad Integration</h1>
                <p className="text-gray-600">Configure and monitor Nagad MFS integration</p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="https://api.nagad.com.bd"
                required
              />
            </div>
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full bg-orange-600 text-white py-2.5 px-4 rounded-lg hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-colors flex items-center justify-center gap-2"
            >
              {saving ? (
                <>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
              </div>
              <select
                value={filterType}
