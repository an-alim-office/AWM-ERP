...
export default function RocketPage() {
  ...
  const fetchConfig = async () => {
    try {
      const res = await fetch("/api/mfs/rocket");
  ...
  const fetchTransactions = async () => {
    try {
      const res = await fetch("/api/mfs/rocket?limit=10");
      const data = await res.json();
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch("/api/mfs/rocket/stats");
      const data = await res.json();
      if (res.ok && data.data) {
        setStats(data.data);
      }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Rocket Integration</h1>
                <p className="text-gray-600">Configure and monitor Rocket MFS integration</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <a
              href="https://www.dutchbanglabank.com/rocket"
              target="_blank"
              rel="noopener noreferrer"
              className="px-4 py-3 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-colors font-medium flex items-center gap-2"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="https://api.rocket.com.bd"
                required
              />
            </div>
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full bg-teal-600 text-white py-2.5 px-4 rounded-lg hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-colors flex items-center justify-center gap-2"
            >
              {saving ? (
                <>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
              <select
                value={filterType}
