/**
 * lib/provisioning/DatabaseProvisioner.ts
 *
 * AWM-ERP — MongoDB Tenant Database Provisioner
 *
 * MongoDB-first tenant provisioning.
 *
...
function loadMongoProvisioningConfig(): MongoProvisioningConfig {
  ...
  if (!uri) {
    throw new TenantError(
      ...
      "MONGODB_URI environment variable পাওয়া যায়নি।",
    );
  }
  ...
  if (!defaultDatabase) {
    throw new TenantError(
      "TENANT_PROVISIONING_FAILED",
      "MONGODB_DB_NAME environment variable পাওয়া যায়নি।",
    );
  }
...
export class DatabaseProvisioner {
  ...
  constructor(
    config: MongoProvisioningConfig = loadMongoProvisioningConfig(),
  ) {
    this.config = config;
  }
  ...
  public async createTenantDatabase(
    ...
    if (dbSelection.provider !== "mongodb") {
      throw new TenantError(
        "TENANT_PROVISIONING_FAILED",
        `এই DatabaseProvisioner শুধুমাত্র 'mongodb' provider সাপোর্ট করে, পাওয়া গেছে: '${dbSelection.provider}'।`,
      );
    }

    switch (dbSelection.isolationStrategy) {
      case "database-per-tenant":
        return this.createDedicatedDatabase(
          companyDetails,
          dbSelection,
        );
      ...
      case "schema-per-tenant":
        return this.createTenantNamespace(
          companyDetails,
          dbSelection,
        );
      ...
      default: {
        ...
        throw new TenantError(
          "TENANT_PROVISIONING_FAILED",
          `অজানা isolation strategy: ${String(
            exhaustiveCheck,
          )}`,
        );
      }
  ...
  private async createDedicatedDatabase(
    ...
    try {
      ...
      return this.buildTenantDbConfig(
        databaseName,
        dbSelection,
        {
          tenantSlug: companyDetails.slug,
  ...
  private async useSharedDatabase(
    ...
    try {
      ...
      throw new TenantError(
        "TENANT_PROVISIONING_FAILED",
        "Shared MongoDB database initialize করা যায়নি।",
        undefined,
        error,
      );
    }
  ...
  private async removeTenantMetadata(
    ...
    try {
      ...
      await db.collection("_tenant_metadata").deleteOne({
        tenantSlug,
      });
    } catch (error: unknown) {
      throw new TenantError(
  ...
  private buildTenantDatabaseName(
    ...
    return `awm_tenant_${safeSlug}`;
  }

  private buildTenantNamespace(
    slug: string,
  ): string {
    const safeSlug = this.sanitizeSlug(slug);
  ...
  private sanitizeSlug(
    ...
    if (!safeSlug) {
      throw new TenantError(
        ...
        "Tenant slug থেকে valid MongoDB identifier তৈরি করা যায়নি।",
      );
    }
